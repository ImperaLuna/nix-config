...
Singleton {
    ...
    function isLauncherEligible(app) {
        if (!app)
            return false;

        // Keep terminal-only utilities out of the graphical launcher.
        if (app.runInTerminal)
            return false;

        // Respect standard desktop-entry visibility flags when present.
